...
public class DishAddActivity extends BaseActivity {
    ...
    private void initTopBar() {
        ...
        tpDishChange.setTbCenterTv(R.string.tian_jia_cai_pin, R.color.color_white);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.iv_activity_dish_change_dish_pic:
                Intent intent = new Intent(DishAddActivity.this, ImageGridActivity.class);
                startActivityForResult(intent, IMAGE_PICKER);
                break;
            case R.id.btn_activity_dish_change_confirm:
                rlNewsdetails.setVisibility(View.VISIBLE);
                btnActivityDishChangeConfirm.setClickable(false);
                addDish();
                break;
        }
    }
...
//        if (pictures.size() == 0) {
//            Toast.makeText(this, R.string.shang_chuan_tu_pian_bu_neng_wei_kong, Toast.LENGTH_SHORT).show();
//            rlNewsdetails.setVisibility(View.GONE);
//            btnActivityDishChangeConfirm.setClickable(true);
//            return;
//        }
        HttpUtils httpUtils = new HttpUtils(Contants.URL_ADD_GOODS) {
            @Override
            public void onError(Call call, Exception e, int id) {
                Toast.makeText(DishAddActivity.this, R.string.please_check_your_network_connection, Toast.LENGTH_SHORT).show();
                rlNewsdetails.setVisibility(View.GONE);
            }
            ...
            public void onResponse(String response, int id) {
                try {
                    ...
                    if (status == 1) {
                        Toast.makeText(DishAddActivity.this, R.string.tian_jia_cheng_gong, Toast.LENGTH_SHORT).show();
                        finish();
                    } else if (status == 9) {
                        Toast.makeText(DishAddActivity.this, R.string.Please_Log_on_again, Toast.LENGTH_SHORT).show();
                        MyApplication.saveLogin(null);
                        startActivity(new Intent(DishAddActivity.this, LoginActivity.class));
                        finish();
                    }
        ...
        httpUtils.addParam("cid", id).addParams("token", login.getToken()).addParams("shopid", login.getShopId())
